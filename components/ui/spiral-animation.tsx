...
class AnimationController {
  ...
  constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number) {
    ...
    this.createStars()
    this.setupTimeline()
    // Start 20% into the cycle (15s × 0.2 = 3s)
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      // @ts-ignore — accessing private field intentionally for 3D projection
...
export function SpiralAnimation({ paused = false, opacity = 1, ready = true }: SpiralAnimationProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const animationRef = useRef<AnimationController | null>(null)
  const [dimensions, setDimensions] = useState({ width: 800, height: 600 })
  // Track current effective-paused state via ref so creation effect can read it
  const shouldPauseRef = useRef(!ready || paused)
  shouldPauseRef.current = !ready || paused
  ...
  useEffect(() => {
    ...
    // Apply initial paused state synchronously — prevents a flash of animation
    if (shouldPauseRef.current) controller.pause()
